...
const submitNewData = () => {
  ...
  return (
    ...
    <div className=' w-[450px] mx-auto h-[300px] mt-60 bg-[#ECEDF6] rounded-xl inset-0 backdrop-blur-sm bg-opacity-25'>
    <p className='font-bold text-xl ml-4 text-[#646681] py-2 mr-12 inline-block'>Add TODO</p>
     ...
     <input type="text" className="p-3 input input-bordered input-accent  w-full max-w-xs mt-2 h-[50px] ml-4" onChange={(x)=>{setNewTitle(x.target.value)}}/>
     <p className='mt-4 font-bold text-l ml-4 text-[#646681]'>STATUS</p>
  
     <select className='p-3 input input-bordered input-accent  w-full max-w-xs mt-2 h-[50px] ml-4 bg-white' onChange={(y)=>{setNewType(y.target.value)}}>
       <option value="Completed">Completed</option>
       <option value="Incompleted">Incompleted</option>
     </select>
